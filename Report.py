...
class Report:
    ...
    def daily(self) -> str:
        ...
        today_expenses = list(filter(lambda x: x.date == date.today().strftime('%Y-%m-%d'), self.organizze.old_transactions))
        this_month_expenses = list(filter(lambda x: this_month_first_day <= datetime.strptime(x.date, '%Y-%m-%d').date() <= this_month_last_day, self.organizze.old_transactions))
        this_week_expenses = list(filter(lambda x: this_week_first_day <= datetime.strptime(x.date, '%Y-%m-%d').date() <= this_week_last_day, self.organizze.old_transactions))
        last_week_expenses = list(filter(lambda x: last_week_first_day <= datetime.strptime(x.date, '%Y-%m-%d').date() <= last_week_last_day, self.organizze.old_transactions))
        ...
        for category, daily_amount in category_today_expenses.items():
            ...
            if budget:
                ...
                percentage = (daily_amount / remaining_budget) * 100 if remaining_budget != 0 else 0.00
            ...
            report += f"Corresponde à: {percentage:.2f}% da meta (Valor restante: R$ {convert_amount_to_decimal(remaining_budget):.2f})\n"
        
        report += "\n"
        
        # Categorias gastas na semana
        ...
        for category, weekly_amount in category_week_to_date_expenses.items():
            ...
            report += f"diferença: {percentage_diff:.2f}%\n"
        ...
        for budget in self.budgets:
            ...
            report += f"meta atingida: {convert_amount_to_decimal((budget_spent / budget_goal) * 100):.2f}% (Valor restante: R$ {convert_amount_to_decimal(budget_remaining):.2f})\n"
        ...
        self.telegram.send_basic_response(report)